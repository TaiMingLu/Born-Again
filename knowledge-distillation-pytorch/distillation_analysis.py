...
def model_analysis(model, dataloader, params, temperature=1., num_classes=10):
    ...
    with torch.no_grad():
        with tqdm(total=len(dataloader)) as t:
            for idx, (data_batch, labels_batch) in enumerate(dataloader):
                ...
                softmax_scores.append(softmax_scores_batch.detach().cpu().numpy())

                preds = torch.argmax(output_batch, dim=1)
                preds_np = preds.detach().cpu().numpy()
                labels_np = labels_batch.detach().cpu().numpy()
                np.add.at(confusion_matrix, (labels_np, preds_np), 1)

                predict_correct_batch = (preds_np == labels_np).astype(int)
                predict_correct.append(np.reshape(predict_correct_batch, (labels_np.size, 1)))
    ...
    return softmax_scores, predict_correct, confusion_matrix
